    ...
    def image_editing_sample(self, img, bs_id=0, tag=None):
        ...
        for it in range(self.args.sample_step):
            ...
            if self.args.use_bm:
                ...
                xs_ = torchsde.sdeint(self.rev_vpsde, x_, ts, method='euler', bm=bm, dt=self.args.denoise_step_size)
            else:
                print(f"The step size is: {self.args.denoise_step_size}")
                xs_ = torchsde.sdeint(self.rev_vpsde, x_, ts, method='euler', dt=self.args.denoise_step_size)
            x0 = xs_[-1].view(x.shape)  # (batch_size, c, h, w)

            # if self.args.whitebox_defense_eval or self.args.adaptive_defense_eval:
            #     os.makedirs(os.path.join(self.args.adv_data_dir, "purified_img", self.args.dataset+self.args.attack_type+str(self.args.batch_seed)), exist_ok=True)
            #     torch.save(x0, os.path.join(self.args.adv_data_dir, "purified_img", self.args.dataset+self.args.attack_type+str(self.args.batch_seed), f'samples_{it}.pt'))
            #     tvu.save_image((x0 + 1) * 0.5, os.path.join(self.args.adv_data_dir, "purified_img", self.args.dataset+self.args.attack_type+str(self.args.batch_seed), f'samples_{it}.png'))
